export default function SupportLoading() {
  return (
    <div className="mx-auto max-w-2xl mt-10 pb-16 animate-pulse">
      ...
      <div className="rounded-2xl border border-border/60 bg-card/40 overflow-hidden divide-y divide-border/50">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="flex items-center justify-between px-5 py-4">
            <div className="h-4 rounded-full bg-muted" style={{ width: `${45 + i * 4}%` }} />
            <div className="h-4 w-4 rounded bg-muted shrink-0 ml-4" />
          </div>
        ))}
      </div>

      <div className="mt-8 flex flex-col items-center gap-3 rounded-2xl border border-border/60 bg-card/40 px-6 py-8">
        ...
        <div className="h-9 w-28 rounded-xl bg-muted mt-1" />
      </div>
    </div>
  )
}
