...
const DEFAULT_TRACKING_FIELDS = [
  { key: 'reps', label: 'Reps', type: 'number' as const },
  { key: 'duration', label: 'Duration (min)', type: 'number' as const },
  { key: 'mood', label: 'Mood', type: 'number' as const },
  { key: 'energy', label: 'Energy', type: 'number' as const },
  { key: 'difficulty', label: 'Difficulty', type: 'number' as const },
  { key: 'note', label: 'Note', type: 'text' as const },
]

export type TrackingFieldType = 'number' | 'text' | 'boolean'
...
export function HabitFormDialog({
  ...
  const [calendarMode, setCalendarMode] = useState<'time' | 'relative'>('time')
  const [habitTime, setHabitTime] = useState('08:00')
  const [habitDuration, setHabitDuration] = useState(30)
  const [relativePosition, setRelativePosition] = useState<'before' | 'after'>('before')
  const [relativeEventId, setRelativeEventId] = useState<number | null>(null)

  const [trackingFields, setTrackingFields] = useState<TrackingField[]>(
    DEFAULT_TRACKING_FIELDS.map((f) => ({ ...f, isDefault: true, enabled: false })),
  )
  ...
  const [goalType, setGoalType] = useState<'field' | 'manual'>('manual')
  const [goalFieldKey, setGoalFieldKey] = useState('')
  const [goalTargetValue, setGoalTargetValue] = useState<number>(10)
  const [goalEndOnReach, setGoalEndOnReach] = useState(false)
  const [goalDescription, setGoalDescription] = useState('')

  const [isPending, setIsPending] = useState(false)

  const { data: calendarEvents } = useQuery({
    queryKey: ['calendar-events-flowline-recurring'],
    queryFn: async () => {
    ...
    enabled: open && calendarOpen && showInCalendar && calendarMode === 'relative',
    staleTime: 60_000,
  })

  const matchingRecurringEvents = (calendarEvents?.docs ?? []).filter((e: any) => {
  ...
  const uniqueRecurringEvents = matchingRecurringEvents.filter(
    (e: any, idx: number, arr: any[]) => arr.findIndex((x: any) => x.id === e.id) === idx,
  )
  ...
  useEffect(() => {
    if (open && initialData) {
      ...
      if ((initialData as any).trackingFields) {
        setTrackingFields((initialData as any).trackingFields)
      }
      if ((initialData as any).goal) {
        const g = (initialData as any).goal as HabitGoal
        setGoal(g)
        setGoalType(g.type)
        setGoalFieldKey(g.fieldKey ?? '')
        setGoalTargetValue(g.targetValue ?? 10)
        setGoalEndOnReach(g.endOnReach ?? false)
  ...
  const addCustomField = () => {
    ...
    setTrackingFields((prev) => [
      ...prev,
      { key, label: newFieldLabel.trim(), type: newFieldType, isDefault: false, enabled: true },
    ])
    setNewFieldLabel('')
  ...
  const goalConfigured = goalOpen && goalDescription.trim()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onSubmit({
      ...
      trackingFields: trackingFields.filter((f) => f.enabled),
      goal: goalConfig,
      ...calendarConfig,
    } as any)
    setIsPending(false)
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-1">
                ...
                <div className="flex gap-1 pt-1">
                  {DAY_OPTIONS.map((day) => (
                    <button
                      ...
                      style={
                        daysOfWeek.includes(day.value) ? { backgroundColor: color } : undefined
                      }
                    >
                      {day.label}
                    </button>
                      ...
                      {uniqueRecurringEvents.length === 0 ? (
                        <p className="text-xs text-muted-foreground/60 py-2 text-center">
                          ...
                          <span className="text-[10px]">
                            Events must match the habit&apos;s frequency
                            {startDate ? ' and start date' : ''}.
                          </span>
                        </p>
                      ) : (
                        <div className="space-y-1">
                          {uniqueRecurringEvents.map((e: any) => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                setRelativeEventId(relativeEventId === e.id ? null : e.id)
                              }
                              className={cn(
          ...
          <Section
            ...
            badge={goalConfigured ? 'Set' : 'Optional'}
          >
            <p className="text-xs text-muted-foreground">
