      ...
      <div className="relative flex flex-1 flex-col overflow-hidden transition-all duration-300">
        ...
        <div className="pointer-events-none absolute inset-0">
          ...
          <div
            ...
            style={{
              background:
                'radial-gradient(circle, oklch(0.55 0.22 290 / 0.08) 0%, transparent 65%)',
            }}
          />
          <div
            ...
            style={{
              background:
                'radial-gradient(ellipse, oklch(0.65 0.18 270 / 0.06) 0%, transparent 70%)',
            }}
          />
        </div>
        ...
        <div className="relative flex items-center justify-between px-5 pt-6 sm:px-10 sm:pt-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                'flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl border backdrop-blur-sm transition-all',
                sidebarOpen
                  ? 'border-violet-500/40 bg-violet-500/10 text-violet-600 dark:text-violet-400'
                  : 'border-border/50 bg-background/60 text-muted-foreground hover:bg-muted hover:text-foreground hover:border-border',
              )}
              aria-label="Saved configs"
            >
              <LayoutList className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
              {configs.length > 0 && !sidebarOpen && (
            ...
            <div className="flex items-center gap-5 sm:gap-6">
              {TODAY_STATS.map((stat) => (
                <div key={stat.label} className="flex flex-col items-center gap-0.5">
                  <span
                    className={cn(
                      'text-base sm:text-lg font-semibold tabular-nums text-foreground leading-none',
