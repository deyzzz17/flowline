...
import { AlertCircle, Check, Loader2, Search, UserPlus, Users, X } from 'lucide-react'
import { api } from '@/api'
...
import { useCreateSharedList } from '@/hooks/list-members/use-create-shared-list'
import {
  LIMIT_ERRORS,
  SAFETY_CAP_ERRORS,
  type LimitError,
...
function ContactRowAvatar({ name, image }: { name: string; image?: string | null }) {
  return (
...
export const NewSharedListClient = () => {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.listMembers.createShared({
        ...
        invites: invitees.map((i) => ({ userId: i.user.id, role: i.role })),
      }),
    onSuccess: (result) => {
      if (!result.ok) {
        if (result.error === LIMIT_ERRORS.SHARED_LISTS_LIMIT) {
          setLimitError(LIMIT_ERRORS.SHARED_LISTS_LIMIT)
          return
        }
        if (result.error === LIMIT_ERRORS.SHARED_LIST_MEMBERS_LIMIT) {
          setLimitError(LIMIT_ERRORS.SHARED_LIST_MEMBERS_LIMIT)
          return
        }
        if (result.error === SAFETY_CAP_ERRORS.SHARED_LISTS_CAP) {
          setCapError(SAFETY_CAP_ERRORS.SHARED_LISTS_CAP)
          return
        }
        if (result.error === SAFETY_CAP_ERRORS.SHARED_LIST_MEMBERS_CAP) {
          setCapError(SAFETY_CAP_ERRORS.SHARED_LIST_MEMBERS_CAP)
          return
        }
        setError(
          result.error === 'DUPLICATE_NAME_ARCHIVED'
            ...
            : result.error === 'DUPLICATE_NAME'
              ...
              : 'Something went wrong while creating the shared list. Please try again.',
        )
        return
      }
      toast.info('Shared list created', {
        description:
          invitees.length > 0
            ? `Invitations sent to ${invitees.length} ${invitees.length === 1 ? 'person' : 'people'}.`
            : 'Your shared list is ready — invite people from its members panel anytime.',
      })
      queryClient.invalidateQueries({ queryKey: ['lists'] })
      router.push(`/lists/${result.value.slug}`)
    },
    onError: () => {
      toast.error('Error while creating the shared list', {
  ...
  return (
    <div className="mx-auto max-w-lg mt-10">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-foreground">New shared list</h1>
        <p className="mt-1.5 text-sm text-muted-foreground">
          Create a list and invite people from your contacts to collaborate on it. You&apos;ll be
          the admin.
        </p>
      </div>
        ...
        <div className="rounded-2xl border border-border/60 bg-card/40 p-6 backdrop-blur-sm space-y-6">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g. Roadtrip, Team backlog..."
              className={cn(
                'h-11 transition-all',
                error &&
                  !name.trim() &&
          ...
          <div className="max-h-40 space-y-1 overflow-y-auto">
            {invitableContacts.length === 0 ? (
              ...
              invitableContacts.map((c) => (
                ...
                >
                  <ContactRowAvatar name={c.user.name} image={c.user.image} />
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-foreground">{c.user.name}</p>
                    <p className="truncate text-xs text-muted-foreground/60">{c.user.email}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => addInvitee(c.user, 'editor')}
