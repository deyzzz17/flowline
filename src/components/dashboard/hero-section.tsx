...
export const HeroSection = () => {
  ...
  return (
    <section className="mx-auto flex min-h-[90vh] max-w-screen-2xl flex-col items-center justify-center px-4 py-24 text-center sm:px-6 lg:px-10">
      <div
    ...
    ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
      ...
      <p
        className={`mt-12 text-xs uppercase tracking-widest text-slate-400 transition-all duration-700 delay-500 dark:text-white/25 ${mounted ? 'opacity-100' : 'opacity-0'}`}
      >
        Trusted by teams at
      </p>
      <div
        className={`mt-4 flex flex-wrap items-center justify-center gap-8 transition-all duration-700 delay-[600ms] ${mounted ? 'opacity-100' : 'opacity-0'}`}
      >
        {['Acme Corp', 'Latitude', 'Nexus', 'Orbit', 'Pulse'].map((name) => (
          <span
            key={name}
            className="text-sm font-semibold text-slate-300 tracking-wide hover:text-slate-500 transition-colors cursor-default dark:text-white/20 dark:hover:text-white/40"
          >
            {name}
          </span>
        ))}
