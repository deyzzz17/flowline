...
export function getItemHeight(item: CalendarItem, viewDate?: Date): number {
  ...
  if (viewDate) {
    const dayStart = new Date(viewDate); dayStart.setHours(0, 0, 0, 0)
    const dayEnd = new Date(viewDate); dayEnd.setHours(24, 0, 0, 0)
  ...
  const durationMin = Math.max(MIN_DURATION_MIN, (effectiveEnd.getTime() - effectiveStart.getTime()) / 60000)
  return minutesToPx(durationMin)
}

export function CalendarEventBlock({
  item, onClickItem, onResizeEnd,
  paddingX = 2, displayHeight, style, viewDate,
  onResizeOverflow,
}: {
  item: CalendarItem
  ...
  const continuesNextDay = viewDate
    ? endDate > (() => { const d = new Date(viewDate); d.setHours(24, 0, 0, 0); return d })()
    : false
  ...
  const endResize = useCallback((clientY: number) => {
    if (!isResizing.current || resizeStartY.current === null) {
      resizeStartY.current = null; return
    }
    const deltaY = clientY - resizeStartY.current
    const newHeightPx = Math.max(minutesToPx(MIN_DURATION_MIN), resizeStartHeight.current + deltaY)
    const newDurationMin = Math.round(pxToMinutes(newHeightPx) / 15) * 15
