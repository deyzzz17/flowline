...
export function GoogleCalendarDialog({ open, onOpenChange }: GoogleCalendarDialogProps) {
  const { isConnected, calendars, connect, disconnect, updateSettings, isConnecting, isDisconnecting } = useGoogleCalendar()
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              Sync your Google Calendar events to Flowline. Your events will be imported automatically every 15 minutes.
            </p>
            <ul className="space-y-1.5 text-sm text-muted-foreground">
              {['View all your Google Calendar events', 'Automatic sync every 15 minutes', 'Read-only — Flowline won\'t modify your Google events'].map((t) => (
            ...
            >
              {isConnecting ? (
                ...
                <><GoogleIcon className="h-4 w-4" />Connect Google Calendar</>
              )}
            </Button>
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-4 py-2">
              ...
              <div className="space-y-1">
                {calendars.map((cal: any) => (
                  <button
                    key={cal.googleId}
                    type="button"
                    onClick={() => updateSettings([{ googleId: cal.googleId, enabled: !cal.enabled }])}
          ...
          <Button
            ...
            onClick={() => { disconnect(undefined); onOpenChange(false) }}
            disabled={isDisconnecting}
          >
            {isDisconnecting ? (
              <><Loader2 className="h-3.5 w-3.5 animate-spin" />Disconnecting...</>
            ) : (
              <><X className="h-3.5 w-3.5" />Disconnect Google Calendar</>
            )}
          </Button>
        </div>
      </DialogContent>
