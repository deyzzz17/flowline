...
function MiniMonth({ year, month, today, getItemsForDate, onClickDay, onClickMonth }: MiniMonthProps) {
  ...
  return (
    <div className="rounded-xl border border-border/40 bg-card/30 p-3 hover:border-border/60 transition-colors">
      ...
      <div className="grid grid-cols-7 mb-1">
        {DAYS_SHORT.map((d, i) => (
          <div key={i} className="text-center text-[9px] font-medium text-muted-foreground/50 py-0.5">
            {d}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-y-0.5">
        {days.map((date, i) => {
          ...
          const eventColors = items
            .slice(0, 3)
            .map((item) => item.type === 'event' ? item.color : (item as any).listColor ?? '#8b5cf6')
              ...
              {hasEvents && (
                <div className="flex gap-0.5 mt-0.5">
                  {eventColors.map((color, ci) => (
                    <span key={ci} className="h-1 w-1 rounded-full" style={{ backgroundColor: color }} />
                  ))}
                </div>
              )}
            </button>
