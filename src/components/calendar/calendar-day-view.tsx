...
function computeColumnsForDay(items: CalendarItem[], viewDate: Date) {
  ...
  for (const group of groups) {
    ...
    for (const item of group) {
      ...
      const itemBottom = itemTop + getItemHeight(item, viewDate)
      let placed = false
...
export function CalendarDayView({
  currentDate, getItemsForDate, onClickSlot, onClickItem, onResizeEnd,
  ...
  return (
    <div className="flex flex-1 flex-col overflow-auto">
      <div className={cn('flex items-center gap-3 px-6 py-4 border-b border-border/40 shrink-0', holiday && 'bg-amber-500/5')}>
        <div className={cn('flex h-10 w-10 items-center justify-center rounded-2xl text-lg font-semibold',
          isToday ? 'bg-violet-600 text-white' : holiday ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400' : 'bg-muted text-foreground')}>
          {currentDate.getDate()}
        </div>
        <div>
          ...
          {holiday ? (
            <p className="text-xs font-medium text-amber-600 dark:text-amber-400">{holiday.localName}</p>
          ) : (
            <p className="text-xs text-muted-foreground/60">{allItems.length} event{allItems.length !== 1 ? 's' : ''}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-1">
        ...
        <div className="flex-1 relative cursor-pointer" style={{ height: totalHeight }}
          onClick={(e) => {
