        ...
        <PopoverTrigger asChild>
          <button type="button"
            ...
            <span className="flex-1 truncate">
              {allDay ? format(value, 'MMM d, yyyy') : `${format(value, 'MMM d, yyyy')} · ${triggerLabel}`}
            </span>
          </button>
        </PopoverTrigger>
          ...
          {!allDay && (
            <div className="border-t border-border/40 p-3 space-y-3">
              <div className="flex items-center gap-1.5">
                ...
                <span className="ml-auto text-sm font-semibold tabular-nums">{currentTimeDisplay}</span>
              </div>
              <div className="grid grid-cols-6 gap-0.5 max-h-28 overflow-y-auto">
                {HOURS.map((h) => (
                  <button key={h} type="button" onClick={() => handleTimeChange(h, timeMinute)}
                    className={cn('rounded-md py-1 text-xs font-medium transition-all',
                      timeHour === h ? 'bg-violet-500/15 text-violet-600 dark:text-violet-400' : 'text-muted-foreground hover:bg-muted')}>
              ...
              <div className="grid grid-cols-4 gap-1">
                {MINUTES.map((m) => (
                  <button key={m} type="button"
                    onClick={() => { handleTimeChange(timeHour, m); setOpen(false) }}
                    className={cn('rounded-md py-1.5 text-xs font-medium transition-all',
                      timeMinute === m ? 'bg-violet-500/15 text-violet-600 dark:text-violet-400' : 'text-muted-foreground hover:bg-muted')}>
                    :{pad(m)}
                  </button>
                ))}
              </div>
...
export function CalendarEventDialog({
  ...
  useEffect(() => {
    ...
    if (isExistingEvent && selectedItem) {
      ...
      setAllDay(false); setColor('#8b5cf6'); setCategoryId(null)
      setMode('create')
    }
    setError(null)
        ...
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-sm">
              <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{ backgroundColor: task.listColor }} />
              Task
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span className="h-1.5 w-1.5 rounded-full shrink-0" style={{ backgroundColor: task.listColor }} />
              {task.listName}
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <CalendarDays className="h-3.5 w-3.5 shrink-0" />
              {format(new Date(task.dueDate), 'EEEE, MMMM d, yyyy')} · {formatTime(new Date(task.dueDate))}
            </div>
          </div>
          <DialogFooter>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                <strong>{ev.title}</strong> will be permanently deleted. This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={handleConfirmDelete} disabled={isDeleting}
                ...
                {isDeleting ? <><Loader2 className="h-3.5 w-3.5 animate-spin" />Deleting...</> : 'Delete event'}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
            ...
            <div className="px-5 pt-4 pb-5 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-foreground">
                  {ev.allDay ? (
                    sameDay ? format(startDate, 'EEEE, MMMM d, yyyy')
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-1">
                ...
                <div className="flex flex-wrap gap-1.5">
                  <button type="button" onClick={() => setCategoryId(null)}
                    className={cn('rounded-full border px-2.5 py-1 text-xs font-medium transition-all',
                      categoryId === null ? 'border-border bg-muted text-foreground' : 'border-border/60 bg-background text-muted-foreground hover:bg-muted')}>
                    None
                  </button>
                  {categories.map((cat) => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <DateTimePicker label="Start" value={startDate} onChange={setStartDate} allDay={allDay} />
            ...
            <DialogFooter className="flex-row gap-2 pt-1">
              ...
              <Button type="submit" disabled={isSaving} className="gap-2">
                {isSaving ? <><Loader2 className="h-3.5 w-3.5 animate-spin" />Saving...</> : mode === 'edit' ? 'Save changes' : 'Create event'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
